...
```mermaid
flowchart LR
    Thin@{ shape: processes, label: "Тонкие клиенты" } --> |"HTTP 9222"| CC[Модуль интеграции]
    Web@{ shape: processes, label: "Веб клиенты" } --> |"HTTP 9222"| CC
    Server[Сервер 1С] <--> |"HTTP 9222, HTTP/HTTPS 80/443*"| CC
...
Сервер 1С передает команды модулю интеграции по порту [!badge 9222] (HTTP).

Тонкий и веб-клиенты 1С также используют порт [!badge 9222] (HTTP) для прямого взаимодействия с модулем интеграции.
...
--- | --- | --- | ---
Сервер 1С | Модуль интеграции | HTTP 9222 | Передача команд
Тонкий клиент 1С | Модуль интеграции | HTTP 9222 | Функции клиентского интерфейса
Веб-клиент 1С | Модуль интеграции | HTTP 9222 | Функции клиентского интерфейса
Модуль интеграции | Веб-сервер 1С | HTTP/HTTPS 80/443 | Передача событий через HTTP-сервис
Сервер 1С | Модуль интеграции | HTTP 9222 | Получение событий в режиме long-poll
...
В этом режиме сервер 1С устанавливает соединение с модулем интеграции по порту [!badge 9222] (HTTP) и использует
...
!!!warning Важно
При отключении регламентных заданий 1С обмен событиями в режиме long-poll прекращается.
!!!
